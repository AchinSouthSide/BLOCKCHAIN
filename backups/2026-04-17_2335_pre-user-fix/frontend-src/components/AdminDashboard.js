                ...
                {allBookings.map(booking => (
                  <div key={booking.id} className="booking-item">
                    <div className="booking-info">
                      ...
                      <p>💰 {booking.totalPrice || '0'} ETH</p>
                      <p>📅 {booking.startTime ? new Date(booking.startTime * 1000).toLocaleString('vi-VN') : 'N/A'}</p>
                      <span className={`status-badge ${booking.status === 3 ? 'completed' : booking.status === 0 ? 'pending' : 'confirmed'}`}>
                        {booking.status === 0 && '⏳ Chờ xác nhận'}
                        {booking.status === 1 && '✅ Đã xác nhận'}
                        {booking.status === 2 && '🎯 Check-in'}
                        {booking.status === 3 && '✔️ Hoàn thành'}
                        {booking.status === 4 && '❌ Huỷ'}
                        {booking.status === 5 && '💸 Hoàn tiền'}
                      </span>
                    </div>
                  </div>
                ))}

...
function OwnerDashboard({ contract, userAddress }) {
  ...
  const loadOwnerData = async () => {
    try {
      ...
      const fieldsData = await ContractService.getFieldsWithStats(contract, userAddress);
  ...
  const handleDeleteField = async (fieldId) => {
    if (!window.confirm('⚠️ Bạn có chắc chắn muốn XÓA sân này? Thao tác này không thể hoàn tác!')) {
      return;
    }
